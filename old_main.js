...
/* 
   ­ƒî┤ Perla de Sabidur├¡a: Firebase es nuestro "Puesto de Vigilancia". 
...
/* 
   ­ƒº╣ El "Conserje Invisible": Este peque├▒o script corre en segundo plano. 
   Su trabajo es asegurarse de que la app abra r├ípido y tenga comida (datos) incluso si cae un diluvio y se va el internet.
*/
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js')
      .then(reg => console.log('­ƒî┤ Selva PWA: Service Worker Activo'))
...
/* 
   ­ƒöù El "Hilo de Ariadna": Mantenemos una conexi├│n inteligente con la base de datos.
   Para evitar el sangrado de lecturas, servimos un cach├® de 15 minutos a los exploradores comunes.
*/
...
async function loadSelvaFlixData() {
  ...
  // 1. Revisar si hay un cach├® v├ílido
  ...
  if (cachedStored && cacheTimestamp && (now - parseInt(cacheTimestamp) < FIFTEEN_MINUTES)) {
    try {
      ...
      // ´┐¢´©Å Vig├¡a Inteligente: Validar que el objeto tenga cara y ojos
      if (!hydratedObject || !Array.isArray(hydratedObject.trending)) {
        throw new Error("Cach├® incompleto o corrupto");
      }

      console.log(`­ƒƒó Objeto rehidratado: { trending: ${hydratedObject.trending.length} }. (0 lecturas)`);
      movieDatabase = hydratedObject;
    } catch (e) {
      console.warn("ÔÜá´©Å Fallo en rehidrataci├│n, limpiando b├║nker para fetch fresco...");
  ...
  if (!hydratedObject) {
    // 2. Si no hay cach├® o caduc├│, pedir a Firebase
    console.log("­ƒöÑ Haciendo expedici├│n a Firebase (Solicitando datos frescos)");
    try {
      ...
      console.error("ÔØî Error en la expedici├│n de datos:", error);
      return; // Muerte s├║bita
    }
  }
...
// Iniciar recolecci├│n al cargar
loadSelvaFlixData();


// ÔöÇÔöÇÔöÇ Filter / Routing ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
function renderChannels(container) {
  ...
  if (liveChannels.length === 0) {
    container.innerHTML = '<p style="color:var(--text-muted);padding:50px;text-align:center;width:100%;">Buscando se├▒al... ­ƒôí</p>';
    return;
  }

  container.innerHTML = liveChannels.map(ch => `
    <div class="tv-card" onclick="window.handleChannelClick('${ch.embed}')">
      <img src="${ch.img}" alt="${ch.title}" style="width:100%;height:100%;object-fit:cover;" onerror="this.src='https://via.placeholder.com/600x400/111/FF7A00?text=SIN+SE├æAL'">
      <div class="tv-info">
...
function handleGlobalSearch(query) {
  ...
  if (query) {
    ...
    else container.insertAdjacentHTML('beforeend', `<p style="padding: 50px; text-align: center; color: var(--text-muted);">No se encontro nada en esta selva... ­ƒòÁ´©ÅÔÇìÔÖé´©Å­ƒÑÑ</p>`);
  } else {
    initApp();
  }
}
...
function _renderCardsInto(container, data) {
  if (!data || data.length === 0) {
    container.innerHTML = '<p style="color:var(--text-muted);padding:30px;">La selva est├í vac├¡a aqu├¡... ­ƒî┐</p>';
    return;
  }
    ...
    const html = chunk.map(item => `
        <div class="movie-card" data-id="${item.id}" onclick="window.handleCardClick('${item.id}')">
          ...
          <div class="card-info">
            ...
            <p class="card-meta">${item.year || 'Estreno'} ÔÇó Ôÿà ${item.rating || '4.8'}</p>
          </div>
        </div>
      `).join('');
...
function renderRow(title, data, seeAllHash = '') {
  ...
  section.innerHTML = `
    <div class="row-header">
      ...
      ${seeAllHash ? `<a href="#${seeAllHash}" class="see-all-btn">Ver todos ÔåÆ</a>` : ''}
    </div>
    <div class="row-container">
      <button class="row-arrow row-arrow-left">ÔùÇ</button>
      <div class="movie-list"></div>
      <button class="row-arrow row-arrow-right">ÔûÂ</button>
    </div>
  `;
...
// Galer├¡a de p├ígina completa con Chunking (v4.4)
function renderGallery(title, groups) {
  ...
  if (container.children.length === 0) {
    container.innerHTML = '<p style="padding:80px;text-align:center;color:var(--text-muted);">La selva est├í vac├¡a por aqu├¡... ­ƒî┐</p>';
  }
}
...
function _renderInventoryRows(items) {
  ...
  const typeIcons = { movie: '­ƒÄ¼', series: '­ƒÅå', live: '­ƒö┤', anime: 'Ôø®´©Å' };
  const langIcons = { 'es-MX': '­ƒç▓­ƒç¢', 'es-PE': '­ƒçÁ­ƒç¬', 'es-ES': '­ƒç¬­ƒç©', 'en-US': '­ƒç║­ƒç©' };

  if (!grid) return;

  if (items.length === 0) {
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; padding:50px; color:var(--text-muted); background:rgba(255,255,255,0.02); border-radius:15px; border:1px dashed #333;">No se encontraron tesoros con ese filtro... ­ƒìâ­ƒòÁ´©ÅÔÇìÔÖé´©Å</div>';
    if (loadMore) loadMore.style.display = 'none';
    if (status) status.innerText = "0 t├¡tulos encontrados.";
    return;
  }
  ...
  if (status) status.innerText = `Mostrando ${visibleItems.length} de ${items.length} t├¡tulos totales.`;
  ...
  grid.innerHTML = visibleItems.map(m => {
    ...
    const icon = typeIcons[m.type] || '­ƒÄ¼';
    const lang = langIcons[m.lang] || '­ƒç▓­ƒç¢';
      ...
      <div class="admin-inv-card" data-id="${m.id}" style="background: rgba(255,255,255,0.03); border: 1px solid ${isBroken ? '#E74C3C' : 'var(--glass-border)'}; border-radius: 12px; padding: 10px; position: relative; transition: transform 0.2s ease; border: 1px solid rgba(255,255,255,0.05);">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; gap: 5px;">
            ...
            <div style="display: flex; gap: 6px;">
                <button onclick="window.editMovie('${m.id}')" title="Editar" style="background: rgba(255,255,255,0.08); border: none; color: white; width:22px; height:22px; display:flex; align-items:center; justify-content:center; border-radius: 5px; cursor: pointer; font-size: 0.65rem; border:1px solid rgba(255,255,255,0.1);">Ô£Å´©Å</button>
                <button onclick="window.deleteMovie('${m.id}')" title="Borrar" style="background: rgba(231,76,60,0.1); border: 1px solid rgba(231,76,60,0.2); color: #E74C3C; width:22px; height:22px; display:flex; align-items:center; justify-content:center; border-radius: 5px; cursor: pointer; font-size: 0.65rem;">­ƒùæ´©Å</button>
            </div>
        </div>
      </div>
    `;
...
window.loadMetrics = async () => {
  ...
  if (log) log.innerText = "Sincronizando con la selva... ­ƒôí";
  ...
  try {
    ...
    if (data.length === 0) {
      log.innerText = "Sin actividad reciente. ­ƒî┤";
      return;
    }
    ...
    log.innerHTML = data.slice(0, 30).map(d => {
      ...
      let emoji = "­ƒæÇ";
      if (d.action === 'play_start' || d.action === 'watch_attempt') { color = "#F1C40F"; emoji = "­ƒÄ¼"; }
      if (d.action === 'page_view') { color = "#3498DB"; emoji = "­ƒº¡"; }
      if (d.action === 'mass_seed') { color = "#E67E22"; emoji = "­ƒÜ£"; }
    ...
    if (log) {
      log.innerHTML = `
            <div style="text-align:center; padding: 20px;">
                <p style="color: #E74C3C; font-weight:bold;">┬íFallo la conexi├│n con las m├®tricas! ­ƒÉÆ</p>
                <p style="font-size:0.7rem; color:var(--text-muted); margin-top:5px;">Error: ${err.message}</p>
                <p style="font-size:0.6rem; color:#444; margin-top:10px;">Nota: Si es la primera vez, Firebase puede tardar 1-2 minutos en generar el ├¡ndice de b├║squeda.</p>
                <button class="btn btn-secondary" style="margin-top:15px; padding:6px 15px; font-size:0.7rem;" onclick="window.loadMetrics()">Reintentar ­ƒöä</button>
            </div>
        `;
    }
  }
...
window.filterInventoryByCategory = () => {
  ...
  if (bulkBtn) {
    if (category === 'broken' && filtered.length > 0) {
      ...
      bulkBtn.innerText = `­ƒùæ´©Å Borrar ${filtered.length} con Error`;
      bulkBtn.onclick = () => window.bulkDeleteMovies(filtered);
    } else if (query === 'nuke' || (type === 'all' && category === 'all' && query === '')) {
      // Solo mostramos el bot├│n de borrar todo en casos espec├¡ficos para evitar accidentes
...
window.bulkDeleteMovies = async (toDelete) => {
  ...
  if (!confirm(`┬┐Est├ís seguro de borrar ${toDelete.length} t├¡tulos de tu selva? ­ƒî┤­ƒùæ´©Å Esta acci├│n es irreversible.`)) return;
  ...
  // ­ƒöÑ Parche de Sincronizaci├│n Real (v4.5.5): Limpiamos cach├® para evitar "Fantasmas"
  ...
  alert(`┬íLimpieza completada! Se fueron ${count} tesoros de la selva.`);
  if (bar) bar.style.width = "0%";
};

window.nukeDatabase = async () => {
  const items = _allInventoryItems;
  if (items.length === 0) { alert("┬íLa selva ya est├í vac├¡a! ­ƒî┤"); return; }

  if (confirm(`ÔÜá´©Å ┬íALERTA ROJA! ÔÜá´©Å\nVas a eliminar ABSOLUTAMENTE TODO el contenido de la base de datos (${items.length} t├¡tulos).\n┬┐EST├üS SEGURO?`)) {
    if (confirm("┬┐Confirmas que quieres quemar toda la selva? ­ƒöÑ Esta acci├│n NO se puede deshacer.")) {
...
window.deleteSelectedCoconas = async () => {
  ...
  if (selected.length === 0) { alert("┬íNo has seleccionado ninguna joya para pelar! ­ƒÉÆ"); return; }

  const confirmed = confirm(`┬┐Est├ís seguro de que quieres eliminar ${selected.length} elementos para siempre? ­ƒöÑ`);
  if (!confirmed) return;
  ...
  if (overlay) overlay.style.display = 'none';
  alert(`┬íLimpieza completada! ${count} elementos eliminados. ­ƒº╣­ƒî┤`);
  if (bar) bar.style.width = "0%";
};
...
window.runBotHealthCheck = async () => {
  ...
  if (items.length === 0) { alert("┬íLa selva est├í vac├¡a! No hay nada que revisar. ­ƒî┤"); return; }

  if (!confirm("­ƒñû ACTIVAR BOT EXPLORADOR:\nOjo: Revisar├® metadatos, im├ígenes y probar├® los enlaces directos (Live TV). ┬┐Continuar? ­ƒöì­ƒî┤")) return;
  ...
  if (statusText) statusText.innerText = "Robot Explorador analizando enlaces y datos... ­ƒñû­ƒöÄ";
  if (overlay) overlay.style.display = 'flex';
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // 2. Revisi├│n de Enlaces Ca├¡dos (Solo links directos como Live TV):
    if (!isBroken && item.embed && item.embed.startsWith('http')) {
      ...
      try {
        ...
        // Si falla el fetch totalmente (ej: dominio no existe), es link ca├¡do de verdad
  ...
  setTimeout(() => {
    if (overlay) overlay.style.display = 'none';
    alert(`­ƒñû INFORME DE LA EXPEDICI├ôN:\n- Revisadas: ${items.length} t├¡tulos.\n- Detectadas con fallas/ca├¡das: ${brokenCount}.\n\nUsa el filtro 'Salud -> Con Errores' para limpiarlas.`);
...
window.searchTMDB = async function (query, isSuggestion = false) {
  ...
  if (!isSuggestion) resultsDiv.innerHTML = '<p style="color: var(--primary);">Buscando en Hollywood... ­ƒôí</p>';

  try {
    let data;
    const lang = document.getElementById('discover-lang')?.value || 'es-MX';
    // Si escriben solo n├║meros, lo buscamos directo por ID (Ej: 1032892)
    if (/^\d+$/.test(query.trim())) {
      ...
      // B├║squeda multi (Pel├¡culas y Series)
    ...
    if (!data.results || data.results.length === 0) {
      resultsDiv.innerHTML = '<p style="color: var(--text-muted);">No encontramos esa joya en la selva ­ƒºÉ</p>';
      return;
    }
    ...
    resultsDiv.innerHTML = (isSuggestion ? '<p style="width:100%; font-size:0.8rem; color:var(--primary); margin-bottom:5px;">­ƒÆí Sugerencias de Imagen:</p>' : '') +
    ...
    resultsDiv.innerHTML = '<p style="color: #E74C3C;">Error al conectar con TMDB (Revisa el ID) ­ƒÉÆ</p>';
  }
}
...
window.selectTMDBMovie = async (index) => {
  ...
  // Operaci├│n IMDB-Latino: Obtener ID real
  try {
    ...
    console.warn("No se pudo obtener IMDB ID en selecci├│n manual.");
    document.getElementById('m-imdb-id').value = "";
  }

  const preview = document.getElementById('m-img-preview');
  if (preview) {
    preview.src = m.poster_path ? (TMDB_IMG_URL + m.poster_path) : 'https://via.placeholder.com/150x220?text=Previsualizaci├│n';
  }

  alert(`Cosechada info de: ${title} ­ƒÑÑ­ƒì╣`);
};
...
function startWarningOverlay(movie) {
  ...
  // L├│gica: Una vez por pel├¡cula/serie al d├¡a
  ...
  if (skipBtn) {
    if (isAdmin) {
      skipBtn.innerText = "ÔÜí Saltar y Comprobar (Modo Admin)";
...
// La l├│gica de servidores y episodios ahora vive en SelvaStream Engine ­ƒì┐

// Exported Actions
window.handleCardClick = (id) => openPlayer(id);

window.deleteMovie = async (id) => {
  if (confirm("┬┐Seguro que quieres eliminar esta joya de la selva? ­ƒÑÑ?")) {
    try {
      await deleteDoc(doc(db, "movies", id));
    } catch (e) {
...
window.editMovie = (id) => {
  ...
  document.getElementById('m-imdb-id').value = movie.imdbId || ""; // Operaci├│n IMDB-Latino
  ...
  // Cambiar botones
  document.getElementById('submit-btn').innerText = "┬íActualizar en la Selva! ­ƒöä";
  document.getElementById('cancel-edit').style.display = "block";

  // Sugerir im├ígenes autom├íticamente al editar
...
window.openLogoSearch = () => {
  ...
  if (!title) { alert("┬íEscribe el nombre del canal primero! ­ƒÉÆ"); return; }
...
window.handleImageUpload = async (file) => {
  ...
  if (file.size > 2 * 1024 * 1024) { alert("┬íUfff! Esa coco├▒a pesa mucho. Usa una imagen menos de 2MB. ­ƒî┤­ƒÉ£"); return; }
  ...
  try {
    ...
    imgInput.value = url;
    preview.src = url;
    alert("┬íSubida con ├®xito a la nube de SelvaFlix! Ôÿü´©Å­ƒªü");
  } catch (err) {
    console.error("Error completo de Firebase:", err);
    alert(`Error al subir: ${err.message}\n\nRECUERDA: Tienes que activar 'Storage' en tu consola de Firebase y poner las reglas en modo prueba o p├║blicas para que funcione. ­ƒÉÆÔÿü´©Å`);
...
async function discoverContent(topic) {
  ...
  status.innerText = `­ƒÑÑ Cosechando sugerencias...`;
  list.innerHTML = '';
  ...
  if (topic === 'live') {
    ...
    const categories = [
      { name: "Deportes ÔÜ¢", img: "https://via.placeholder.com/400x225/111/fff?text=DEPORTES+TV", embed: "" },
      { name: "Cine y Pelis ­ƒì┐", img: "https://via.placeholder.com/400x225/111/fff?text=CINE+TOTAL", embed: "" },
      { name: "Noticias ­ƒôí", img: "https://via.placeholder.com/400x225/111/fff?text=NOTICIAS+24/7", embed: "" },
      { name: "Cultural ­ƒî┐", img: "https://via.placeholder.com/400x225/111/fff?text=CULTURA+Y+NATURA", embed: "" }
    ];
    ...
    const peruvianChannels = [
      ...
      { name: "Am├®rica TV", img: "https://logodownload.org/wp-content/uploads/2018/11/america-tv-logo.png", embed: "https://ejemplo.com/m3u8-player?url=https://stream.america.pe/live.m3u8" },
      ...
      { name: "TV Per├║", img: "https://upload.wikimedia.org/wikipedia/commons/2/29/TV_Per%C3%BA_logo_2020.png", embed: "https://stream.tvperu.gob.pe/live.m3u8" }
    ];

    status.innerText = "­ƒô║ Categor├¡as y Canales Sugeridos:";
    list.innerHTML = `
      <p style="grid-column: 1/-1; font-size: 0.7rem; color: var(--text-muted); margin-top: 10px;">Iconos de Categor├¡a (Para canales manuales):</p>
      ${categories.map(c => `
      ...
      ${peruvianChannels.map(ch => `
        <div style="background: rgba(255,255,255,0.05); padding: 8px; border-radius: 8px; display: flex; align-items: center; gap: 8px; border: 1px solid var(--glass-border);">
          ...
          <div style="flex: 1; overflow: hidden;">
            ...
            <button onclick="window.quickSeedManual(${JSON.stringify(ch).replace(/"/g, '&quot;')}, 'live')" style="background: #2ECC71; border: none; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.6rem; cursor: pointer;">Ô×ò Sembrar</button>
          </div>
        </div>
      `).join('')}
    `;
  ...
  try {
    ...
    if (newItems.length === 0) {
      status.innerText = "­ƒìâ No hay nada nuevo por aqu├¡ con esos filtros.";
      return;
    }

    status.innerText = `­ƒÆí Toca para sembrar (Mostrando ${newItems.length} nuevas):`;
    list.innerHTML = newItems.slice(0, 12).map(s => `
      <div style="background: rgba(255,255,255,0.05); padding: 8px; border-radius: 8px; display: flex; align-items: center; gap: 8px; border: 1px solid var(--glass-border);">
        ...
        <div style="flex: 1; overflow: hidden;">
          ...
          <button onclick="window.quickSeedContent(${JSON.stringify(s).replace(/"/g, '&quot;')}, '${topic}')" style="background: var(--primary); border: none; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.6rem; cursor: pointer;">Ô×ò Sembrar</button>
        </div>
      </div>
    `).join('');
  } catch (err) {
    status.innerText = "ÔØî Error al conectar con TMDB";
  }
}

window.quickSeedContent = async (s, type) => {
  const exists = movieDatabase.trending.find(m => m.tmdbId == s.id);
  if (exists) { alert(`┬í"${s.title || s.name}" ya estaba en el jard├¡n!`); return; }

  // Operaci├│n IMDB-Latino (v4.6.0): Obtener imdbId heredado
  let imdbId = "";
  try {
    ...
    console.warn("No se pudo obtener IMDB ID para siembra r├ípida.");
  }
...
window.massSeedMovies = async (contentType) => {
  ...
  if (!list || !status || !container || !confirmBtn) {
    ...
    alert('Error interno: recarga la p├ígina e int├®ntalo de nuevo.');
    return;
  }
  ...
  // Comparaci├│n robusta: acepta tmdbId como string o number
  ...
  const maxExtraAttempts = 5; // Si la p├ígina 1 est├í llena de duplicados, probamos hasta 5 p├íginas m├ís
  let pagesSearched = 0;
  let totalPagesTried = 0;

  try {
    // Buscamos las p├íginas pedidas, y si todas est├ín duplicadas probamos m├ís autom├íticamente
    for (let attempt = 0; pagesSearched < pages && attempt < pages + maxExtraAttempts; attempt++) {
      const pageNum = attempt + 1;
      status.innerText = `­ƒöì Buscando p├ígina ${pageNum}... (${pendingSeeds.length} nuevas encontradas)`;
      ...
      const res = await fetch(url);
      if (!res.ok) throw new Error(`TMDB respondi├│ con error ${res.status}`);
      const data = await res.json();
      totalPagesTried++;

      if (!data.results || data.results.length === 0) break; // No hay m├ís p├íginas
      ...
      for (const s of data.results) {
        ...
        if (!existingIds.has(tmdbIdStr) && (s.poster_path || s.backdrop_path)) {
          pendingSeeds.push({
            title: s.title || s.name || 'Sin t├¡tulo',
      ...
      if (foundNew > 0) pagesSearched++; // Solo contamos p├íginas que aportaron algo nuevo
    }
    ...
    if (pendingSeeds.length === 0) {
      status.innerHTML = `
        <span>­ƒìâ Todas las pel├¡culas populares de TMDB ya est├ín en tu base de datos.</span><br>
        <span style="font-size:0.7rem; color: var(--text-muted);">Prueba cambiando el <b>a├▒o</b> o el <b>g├®nero</b> para encontrar contenido nuevo.</span>
      `;
      return;
    }

    status.innerText = `Ô£à ┬í${pendingSeeds.length} coconas nuevas! Desmarca las que no quieras:`;
    confirmBtn.style.display = 'block';
    confirmBtn.innerText = `Ô£à Sembrar ${pendingSeeds.length} Coconas`;
    ...
    list.innerHTML = pendingSeeds.map((s, idx) => `
      <div style="background: rgba(255,255,255,0.05); padding: 8px; border-radius: 8px; display: flex; align-items: center; gap: 8px; border: 1px solid var(--glass-border);">
        ...
        <div style="flex: 1; overflow: hidden;">
          ...
          <p style="font-size: 0.6rem; color: var(--text-muted);">${s.year} ┬À ${s.type}</p>
        </div>
      </div>
    `).join('');
    ...
    status.innerHTML = `
      <span style="color:#E74C3C;">ÔØî Error: ${err.message}</span><br>
      <span style="font-size:0.7rem; color:var(--text-muted);">Verifica tu conexi├│n e int├®ntalo de nuevo.</span>
    `;
  }
};
...
window.updateSeedCount = () => {
  ...
  if (btn) {
    btn.innerText = `Ô£à Sembrar ${checked} Coconas seleccionadas`;
...
window.confirmBatchSeed = async () => {
  ...
  const confirmed = confirm(`┬┐Sembrar estas ${checks.length} coconas ahora? ­ƒÜ£­ƒî┤`);
  if (!confirmed) return;
  ...
  if (statusText) statusText.innerText = "Sembrando nuevas Coconas... ­ƒî┤Ô£¿";
  if (overlay) overlay.style.display = 'flex';
  ...
  if (overlay) overlay.style.display = 'none';
  alert(`┬íSiembra masiva completada! ${count} elementos a├▒adidos. ­ƒî┤­ƒì┐`);
  ...
  section.innerHTML = itemsToShow.map(item => `
    <div class="hero-card" onclick="window.openPlayer('${item.id}')" style="flex: 1; min-width: 300px; height: 300px; background-image: linear-gradient(to top, rgba(0,0,0,0.9), rgba(0,0,0,0.1)), url('${item.img}'); background-size: cover; background-position: center 20%; border-radius: 16px; position: relative; cursor: pointer; border: 1px solid var(--glass-border); transition: transform 0.3s ease; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
      <div style="position: absolute; bottom: 20px; left: 20px; right: 20px;">
        ...
        <p style="color: var(--primary); font-size: 0.9rem; font-weight: bold; text-shadow: 0 1px 3px rgba(0,0,0,0.8);">Ô¡É ${item.rating || '4.8'} ÔÇó ${item.year || '2024'}</p>
        <button class="btn btn-primary" style="margin-top: 10px; padding: 8px 20px; font-size: 0.8rem;">ÔûÂ Reproducir</button>
      </div>
    </div>
  `).join('');
}
...
function initApp(filterType = '', genreId = '') {
  ...
  if (container) {
    container.innerHTML = '';
    renderSkeletons(); // Flash visual instant├íneo
  }

  // Actividad: Vista de p├ígina
  collectUserData("page_view", { page: filterType || 'home' });

  // ORDEN INTELIGENTE: Salud -> Fecha de Creaci├│n
  ...
  if (heroPool.length > 0) {
    ...
    updateHeroCarousel();
    // La rotaci├│n iniciar├í al final para no estorbar el pintado inicial
  } else {
  ...
  if (filterType === 'movies') {
    ...
    if (movies.length > 0) {
      renderGallery('­ƒÄ¼ Pel├¡culas', [{ label: `­ƒÄ¼ Pel├¡culas${genreId ? ' ┬À filtradas' : ''}`, items: movies }]);
    } else {
      if (container) container.innerHTML = '<p style="padding:80px;text-align:center;color:var(--text-muted);">No hay pel├¡culas con ese filtro ­ƒî┐</p>';
    }

  } else if (filterType === 'series') {
    const series = allContent.filter(c => c.type === 'series' || c.type === 'tv');
    const anime = allContent.filter(c => c.type === 'anime');
    renderGallery('­ƒÅå Series & Anime', [
      { label: `­ƒÅå Series${genreId ? ' ┬À filtradas' : ''}`, items: series },
      { label: `Ôø®´©Å Anime`, items: anime }
    ]);

  } else if (filterType === 'live') {
    if (container) container.innerHTML = '';
    renderRow('Canales en Vivo ­ƒö┤', []);
    ...
    if (container) container.innerHTML = ''; // Los skeletons cumplieron su misi├│n
    ...
    if (releases.length > 0) renderRow('Ô£¿ Lo m├ís nuevo', releases, 'movies');
    if (movies.length > 0) renderRow('­ƒÄ¼ Pel├¡culas', movies, 'movies');
    if (series.length > 0) renderRow('­ƒÅå Series', series, 'series');
    if (anime.length > 0) renderRow('Ôø®´©Å Anime', anime, 'series');
    if (liveChannels.length > 0) {
      renderRow('­ƒö┤ Canales en Vivo', [], 'live');
      const sec = container.lastElementChild;
      const list = sec.querySelector('.movie-list');
      list.id = 'main-channels';
      renderChannels(list);
  ...
  // ­ƒÜÇ Encendido del motor de rotaci├│n (al final para liberar el hilo principal)
  if (heroPool.length > 3) {
  ...
  if (channels.length === 0) {
    container.innerHTML = `
      <div style="grid-column: 1/-1; padding: 40px; text-align: center; background: rgba(255,255,255,0.02); border-radius: 20px; border: 1px dashed #333;">
        <p style="color:var(--text-muted); margin-bottom:15px;">La selva est├í en silencio... No hay canales a├║n.</p>
        <button onclick="window.location.hash='admin'; window.switchAdminTab('inventory');" class="btn btn-primary" style="font-size:0.8rem;">­ƒÜ£ Sembrar Canales</button>
      </div>
    `;
    return;
  }
    ...
    <div class="movie-card live-card" onclick="window.handleCardClick('${ch.id}')" style="min-width: 200px; height: 120px;">
       <div class="card-img-wrapper" style="height: 100%;">
          ...
          <div class="card-overlay">
            <div class="play-btn-circle"><span>ÔûÂ</span></div>
          </div>
...
window.suggestTVChannels = () => {
  ...
  container.style.display = 'block';
  status.innerText = "­ƒô║ Canales de TV Sugeridos (Links M3U8 p├║blicos):";
  ...
  list.innerHTML = suggestions.map(s => `
     <div style="background: rgba(255,255,255,0.05); padding: 8px; border-radius: 8px; display: flex; align-items: center; gap: 8px; border: 1px solid var(--glass-border);">
        ...
        <div style="flex: 1;">
          ...
          <button onclick="window.quickSeedManual(${JSON.stringify(s).replace(/"/g, '&quot;')}, 'live')" style="background: #2ECC71; border: none; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; cursor: pointer;">Ô×ò Agregar Canal</button>
        </div>
      </div>
  `).join('');
};
...
window.discoverM3U = async () => {
  ...
  container.style.display = 'block';
  status.innerText = "­ƒôí Cargando canales desde GitHub (IPTV-Org Latam)...";
  list.innerHTML = '<p style="grid-column: 1/-1; text-align:center;">­ƒöì Extrayendo el coraz├│n de los sat├®lites...</p>';
  ...
  try {
    ...
    if (channels.length === 0) { status.innerText = "ÔØî No se encontraron canales v├ílidos."; return; }

    status.innerText = `Ô£à Encontrados ${channels.length} canales en Latinoam├®rica:`;
      ...
      return `
        <div style="background: rgba(255,255,255,0.05); padding: 8px; border-radius: 8px; display: flex; align-items: center; gap: 8px; border: 1px solid var(--glass-border);">
            ...
            <div style="flex: 1; overflow: hidden;">
              ...
              <button onclick="window.quickSeedManual(${JSON.stringify(cleanData).replace(/"/g, '&quot;')}, 'live')" style="background: #2ECC71; border: none; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.55rem; cursor: pointer;">Ô×ò Agregar</button>
            </div>
        </div>
      `;
    }).join('');
  } catch (err) {
    status.innerText = "ÔØî Error al cargar lista de GitHub.";
  }
};
...
window.autoSuggestLogo = async () => {
  ...
  const imgInput = document.getElementById('m-img');
  const preview = document.getElementById('m-img-preview');
  if (!title) { alert("┬íDime el nombre del canal primero! ­ƒÉÆ"); return; }

  const originalText = title;
  imgInput.value = "­ƒöì Buscando logo...";
  ...
  try {
    ...
    // Si falla, abrimos b├║squeda en duckduckgo im├ígenes para el usuario
...
document.addEventListener('DOMContentLoaded', () => {
  // Nota: handleRouting se dispara autom├íticamente cuando loadSelvaFlixData termina de cargar
  ...
  document.getElementById('m-img').addEventListener('input', (e) => {
    const preview = document.getElementById('m-img-preview');
    if (preview) {
      preview.src = e.target.value || 'https://via.placeholder.com/150x220?text=Previsualizaci├│n';
    }
  });

  // Pegado directo de im├ígenes (Clipboard)
  ...
  document.getElementById('movie-form').addEventListener('submit', async (e) => {
    ...
    if (!title) { alert('┬íFalta el t├¡tulo! ­ƒî┤'); return; }
    if (!img) { alert('┬íFalta la imagen del p├│ster! Busca una en TMDB o pega la URL. ­ƒû╝´©Å'); return; }

    const movieData = {
      title,
      img,
      tmdbId: document.getElementById('m-tmdb-id').value.trim(),
      imdbId: document.getElementById('m-imdb-id').value.trim(), // Operaci├│n IMDB-Latino
    ...
    submitBtn.innerText = 'Guardando... ÔÅ│';

    try {
      if (dbId) {
        await updateDoc(doc(db, "movies", dbId), movieData);
        alert('┬íActualizaci├│n Exitosa! ­ƒî┤­ƒöä');
      } else {
        movieData.createdAt = Date.now();
        await addDoc(moviesCol, movieData);
        alert('┬íCosecha Exitosa! ­ƒî┤­ƒì┐');
      }

      // Reset form
      e.target.reset();
      document.getElementById('m-db-id').value = "";
      document.getElementById('m-imdb-id').value = ""; // Operaci├│n IMDB-Latino
      document.getElementById('m-img-preview').src = 'https://via.placeholder.com/150x220?text=Previsualizaci├│n';
      document.getElementById('cancel-edit').style.display = "none";
      document.getElementById('tmdb-results').innerHTML = '';

    } catch (error) {
      console.error("Error guardando en Firebase:", error);
      alert(`Error al guardar: ${error.message} ­ƒÉÆ`);
    } finally {
      submitBtn.disabled = false;
      submitBtn.innerText = '┬íGuardar en la Selva! ­ƒî┤Ô£¿';
    }
  });
  ...
  document.getElementById('cancel-edit').addEventListener('click', () => {
    ...
    document.getElementById('m-imdb-id').value = ""; // Operaci├│n IMDB-Latino
    document.getElementById('m-img-preview').src = 'https://via.placeholder.com/150x220?text=Previsualizaci├│n';
    document.getElementById('submit-btn').innerText = "┬íGuardar en la Selva! ­ƒî┤Ô£¿";
    document.getElementById('cancel-edit').style.display = "none";
  });
